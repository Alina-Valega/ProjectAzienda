...
public class DatabaseOperations {
    ...
    public  List getAllSchoolDetails() {
        Query queryObj = entityMgrObj.createNamedQuery("SchoolEntityManager.findAll",SchoolEntityManager.class);
    ...
    public static String createNewSchool(String name) {
        ...
        newSchoolObj.setIdSchool(getMaxSchoolId());
        newSchoolObj.setName(name);
        entityMgrObj.merge(newSchoolObj);
    ...
    public static String deleteSchoolDetails(int schoolId) {
        ...
        if(isSchoolIdPresent(schoolId)) {
            deleteSchoolObj.setIdSchool(schoolId);
    ...
    public static String updateSchoolDetails(int schoolId, String updatedSchoolName) {
        ...
        if(isSchoolIdPresent(schoolId)) {
            Query queryObj = entityMgrObj.createQuery("UPDATE SchoolEntityManager s SET s.name=:name WHERE s.idSchool= :id");         
    ...
    private static int getMaxSchoolId() {
        int maxSchoolId = 1;
        Query queryObj = entityMgrObj.createQuery("SELECT MAX(s.idSchool)+1 FROM SchoolEntityManager s");
    ...
    private static boolean isSchoolIdPresent(int schoolId) {
        ...
        Query queryObj = entityMgrObj.createQuery("SELECT s FROM SchoolEntityManager s WHERE s.idSchool = :id");
        queryObj.setParameter("id", schoolId);
